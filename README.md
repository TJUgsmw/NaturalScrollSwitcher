...
macOS 的“自然滚动”是全局设置，系统没有给鼠标和触控板分别保存两个开关。NaturalScrollSwitcher v0.6.4 会根据当前权限自动选择最可用的运行方式：权限完整时写系统设置并用事件修正补偿切换瞬间，只有输入监控时回退到全局设置切换，没有监听权限时仍保留手动切换。

- 检测到普通鼠标滚轮时，会按鼠标偏好写入 macOS 自然滚动设置；默认是关闭。
- v0.6.0 起会同时监听 HID 鼠标滚轮输入，普通 USB/蓝牙滚轮鼠标不再只依赖 macOS 的连续滚动字段判断。
- 检测到触控板连续滚动或手势时，会按触控板偏好写入 macOS 自然滚动设置；默认是开启。
- 事件级修正只用于系统设置尚未跟上鼠标偏好的过渡瞬间，避免已经切换后继续双重反转。
...
Magic Mouse 的滚动事件更接近触控设备，v0.6.4 暂不承诺稳定识别。普通 USB/蓝牙滚轮鼠标是当前主要支持目标。
...
```text
dist/NaturalScrollSwitcher-0.6.4-macos-<arch>.zip
dist/NaturalScrollSwitcher-0.6.4-macos-<arch>.dmg
dist/checksums.txt
```
...
- Event-level correction only covers the transition window before the system setting catches up.
...
Magic Mouse is not a stable v0.6.4 target because its scroll events are closer to touch devices than ordinary mouse wheels.
