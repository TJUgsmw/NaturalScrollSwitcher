...
macOS 的“自然滚动”是全局设置，系统没有给鼠标和触控板分别保存两个开关。NaturalScrollSwitcher v0.5.0 使用更稳定的策略：系统设置保持为触控板偏好，普通鼠标滚轮在事件层按鼠标偏好修正方向。
...
- v0.5.0 起包含最近动作诊断，用来确认鼠标滚动是否被修正。
...
事件级滚动修正需要输入监控和辅助功能权限。菜单里会显示当前权限状态。
...
- `切换到鼠标: 自然滚动开启/关闭`：显示鼠标偏好，并准备按事件级规则修正鼠标滚轮。
- `切换到触控板: 自然滚动开启/关闭`：按触控板偏好同步系统自然滚动基线。
- `最近动作`：显示最近滚动是否被修正，例如“已修正鼠标滚动”或“触控板滚动未修正”。
- `打开输入监控设置`：打开 macOS 输入监控权限页面。
- `打开辅助功能设置`：打开 macOS 辅助功能权限页面。

Magic Mouse 的滚动事件更接近触控设备，v0.5.0 暂不承诺稳定识别。普通 USB/蓝牙滚轮鼠标是当前主要支持目标。
...
```text
dist/NaturalScrollSwitcher-0.5.0-macos-<arch>.zip
dist/NaturalScrollSwitcher-0.5.0-macos-<arch>.dmg
dist/checksums.txt
```
...
NaturalScrollSwitcher is a small macOS menu bar utility that keeps trackpad scrolling aligned with the macOS natural scrolling setting and corrects ordinary mouse wheel events directly when needed.

macOS exposes natural scrolling as one global setting. Since changing that setting can be delayed, v0.5.0 keeps the global setting aligned with the trackpad preference and applies event-level correction for ordinary mouse wheels.
...
- Recent-action diagnostics for corrected mouse scroll and pass-through trackpad scroll.
...
On first launch, grant both Input Monitoring and Accessibility permissions. Event-level correction requires both permissions.
...
Magic Mouse is not a stable v0.5.0 target because its scroll events are closer to touch devices than ordinary mouse wheels.
