...
macOS 的“自然滚动”是全局设置，系统没有给鼠标和触控板分别保存两个开关。NaturalScrollSwitcher v0.6.7 会根据当前权限自动选择最可用的运行方式：权限完整时同步系统设置并用事件修正补偿切换瞬间，只有输入监控时回退到全局设置切换，没有监听权限时仍保留手动切换。
...
Magic Mouse 的滚动事件更接近触控设备，v0.6.7 暂不承诺稳定识别。普通 USB/蓝牙滚轮鼠标是当前主要支持目标。
...
```text
dist/NaturalScrollSwitcher-0.6.7-macos-<arch>.zip
dist/NaturalScrollSwitcher-0.6.7-macos-<arch>.dmg
dist/checksums.txt
```
...
Magic Mouse is not a stable v0.6.7 target because its scroll events are closer to touch devices than ordinary mouse wheels.
