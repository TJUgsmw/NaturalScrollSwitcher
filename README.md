...
Magic Mouse 的滚动事件更接近触控设备，v0.4.0 暂不承诺稳定识别。普通 USB/蓝牙滚轮鼠标是当前主要支持目标。
...
```text
dist/NaturalScrollSwitcher-0.4.0-macos-<arch>.zip
dist/NaturalScrollSwitcher-0.4.0-macos-<arch>.dmg
dist/checksums.txt
```
...
Magic Mouse is not a stable v0.4.0 target because its scroll events are closer to touch devices than ordinary mouse wheels.
