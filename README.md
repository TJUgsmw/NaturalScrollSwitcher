...
macOS 的“自然滚动”是全局设置，系统没有给鼠标和触控板分别保存两个开关。NaturalScrollSwitcher v0.5.1 会根据当前权限自动选择最可用的运行方式：权限完整时做事件级修正，只有输入监控时回退到全局设置切换，没有监听权限时仍保留手动切换。
...
Magic Mouse 的滚动事件更接近触控设备，v0.5.1 暂不承诺稳定识别。普通 USB/蓝牙滚轮鼠标是当前主要支持目标。
...
```text
dist/NaturalScrollSwitcher-0.5.1-macos-<arch>.zip
dist/NaturalScrollSwitcher-0.5.1-macos-<arch>.dmg
dist/checksums.txt
```
...
macOS exposes natural scrolling as one global setting. v0.5.1 chooses the best available runtime mode: event-level correction when both permissions are available, global setting fallback when only Input Monitoring is available, and manual-only switching when automatic input detection is unavailable.
...
Magic Mouse is not a stable v0.5.1 target because its scroll events are closer to touch devices than ordinary mouse wheels.
